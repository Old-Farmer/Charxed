...
Result TextWindow::DeleteAtCursor() {
...
Result TextWindow::NewLineUnderCursorline() {
    CHX_ASSERT(!area_.IsSelectionActive());
    return TryAutoIndent(
        {cursor_->pos.line,
...
Result TextWindow::Replace(const Range& range, std::string_view str) {
...
void TextWindow::AttachBuffer(Buffer* buffer) {
    DetachBuffer();
...
void TextWindow::SetJumpPoint() {
    InsertJumpHistory();
    MoveJumpHistoryCursorForwardAndTruncate();
}

void TextWindow::JumpForward() {
...
void TextWindow::SaveView() {
    area_.b_view_->SaveCursorState(cursor_);
    stored_b_view_ = *area_.b_view_;
}

void TextWindow::RestoreView() {
    *area_.b_view_ = stored_b_view_;
    area_.b_view_->RestoreCursorState(cursor_, area_.buffer_);
}

int64_t TextWindow::AllocId() noexcept { return cur_window_id_++; }

int64_t TextWindow::cur_window_id_ = 0;
